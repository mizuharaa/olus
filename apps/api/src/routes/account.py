...
def origin_guard(request: Request):
    ...
    allowed = os.environ.get(
        ...
        "https://olus.sh,https://www.olus.sh",
    ).split(",")
...
def login(body: Login, request: Request, response: Response):
    ...
    with database() as db:
        ...
        if not row or not valid:
            raise HTTPException(401, "Email or password is incorrect")
        db.execute("DELETE FROM account_attempts WHERE scope=?", (digest("email:" + email),))
...
def confirm_password(db, actor, password):
    ...
    db.execute("DELETE FROM account_attempts WHERE scope=?", (digest("email:" + actor["email"]),))

...
class WhatIfRequest(BaseModel):
    model_config = ConfigDict(extra="forbid")
    decision_locks: list[dict] = Field(min_length=1, max_length=2000)
...
@router.post("/runs/{run_id}/what-if", status_code=202)
async def what_if_run(
    run_id: str, payload: WhatIfRequest, request: Request, account=Depends(require_account)
):
    return await owned_call(manager(request).what_if, account["id"], run_id, payload.decision_locks)
...
async def explain_run(
    ...
    return await asyncio.to_thread(
        ...
        event_kind=economic_event_kind(result.get("active_events") or []),
        top_n=payload.top_n,
    )

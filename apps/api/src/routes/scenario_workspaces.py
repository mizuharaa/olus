...
class CreateScenario(BaseModel):
    ...
    template: Literal["validated_reference", "legacy_network"] = "validated_reference"
...
def create_scenario(payload: CreateScenario, request: Request, account=Depends(require_account)):
    # The historical large fixture contains overlapping duties. Keep it opt-in;
    # new drafts start from a small consistent, explicitly fictional network.
    config = (
        ScenarioConfig(**reference_scenario())
        if payload.template == "validated_reference"
        else ScenarioConfig(

...
def _worker(config: dict, output):
    ...
    async def solve():
        engine = SimulationEngine(
            ...
            decision_locks=config.get("decision_locks"),
        )
        ...
        if not config["disruptions"]:
            update = await engine.solve_current(predictor, optimizer, WeatherSnapshot())
...
class RunManager:
    ...
    def __init__(self, path: str | Path, max_workers: int = 2):
        ...
        self.db.commit()
        self.workers: dict[str, tuple[BaseProcess, threading.Thread]] = {}
    ...
    def get(self, owner, run_id, include_input=False):
        with self.lock:
            ...
            if data.get("parent_run_id") and data["status"] == "completed":
                parent = self._get(owner, data["parent_run_id"])
                feasible = {"optimal", "feasible", "heuristic"}
                parents = {
                    p["plan_id"]: p for p in (parent.get("result") or {}).get("recovery_plans", [])
                }
                comparisons = []
                for child in data["result"].get("recovery_plans", []):
                    before = parents.get(child["plan_id"], {})
                    if child.get("status") not in feasible or before.get("status") not in feasible:
                        continue
                    complete = all(
                        p.get("cost_breakdown", {}).get("passenger_delay_complete", False)
                        for p in (before, child)
                    )
                    comparisons.append(
                        {
                            "plan_id": child["plan_id"],
                            "passenger_comparison_complete": complete,
                            **{
                                f"{metric}_delta": child.get(metric, 0) - before.get(metric, 0)
                                for metric in ("total_cost_usd", "total_co2_kg")
                            },
                            "total_passenger_delay_minutes_delta": (
                                child.get("total_passenger_delay_minutes", 0)
                                - before.get("total_passenger_delay_minutes", 0)
                                if complete
                                else None
                            ),
                        }
                ...
                data["comparison"] = {
                    "basis": "modeled_child_minus_parent",
                    "estimate_only": True,
                    "plans": comparisons,
                }
        if data["status"] not in TERMINAL:
    ...
    def what_if(self, owner, parent_run_id, decision_locks):
        """Fork frozen inputs; the original run is never applied or modified."""
        from src.optimizer.feasibility import validate_decision_locks

        with self.lock:
            parent = self._get(owner, parent_run_id)
            if parent["status"] != "completed" or not parent.get("result"):
                raise RuntimeError("Only a completed run can be forked")
            config = json.loads(json.dumps(parent["input"]))
            if not decision_locks:
                raise ValueError("At least one decision lock is required")
            config["decision_locks"] = validate_decision_locks(
                decision_locks, config["schedule"], config["aircraft"]
            )
            return self.start(
                owner,
                {"id": parent["scenario_id"], "name": parent["scenario_name"], "config": config},
                config.get("weather"),
                parent_run_id=parent_run_id,
                parent_input_hash=parent["input_hash"],
            )

    def start(self, owner, scenario, weather=None, *, parent_run_id=None, parent_input_hash=None):
    ...
    def apply(self, owner, run_id, plan_id):
        ...
        with self.lock:
            ...
            result = data["result"]
            if plan_id and not any(plan["plan_id"] == plan_id for plan in result["recovery_plans"]):
                raise ValueError("Plan not found in this run")
            engine = SimulationEngine(
                result["schedule"],
                result["aircraft"],
                result["crew_pairings"],
                crew_members=result.get("crew_members"),
                decision_locks=result.get("decision_locks"),
            )

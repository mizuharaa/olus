...
def test_real_run_progress_private_apply_and_history(tmp_path):
    ...
    try:
        ...
        with manager.lock:
            ...
            with pytest.raises(RuntimeError, match="already have a running solve"):
                manager.start("owner", fixture)
        expected_input = {**original["config"], "weather": {}}
        assert (
            run["input_hash"]
            == hashlib.sha256(
...
def test_cancel_terminates_process_and_preserves_terminal_state(tmp_path):
    ...
    try:
        ...
        with manager.lock:
            ...
            replacement = manager.start("owner", scenario())
        manager.cancel("owner", replacement["id"])
...
def test_crew_audit_uses_members_and_marks_missing_inputs_unknown():
    ...
    row = next(r for r in audit["rows"] if r["crew_id"] == "CA01" and r["rule"] == "modeled-7d")
    assert row["value"] == 18 * 60 + 135
    assert row["slack"] == 60 * 60 - row["value"]
    assert row["inputs"]["proposed_flight_minutes"] == 135

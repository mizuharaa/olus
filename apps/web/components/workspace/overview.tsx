...
export function Overview(){
 ...
 return <div className={s.console} data-olus-console data-compare={compare} style={{"--recovery-height":`${recoverySize.size}px`,"--timeline-height":`${timelineSize.size}px`} as CSSProperties}>
  ...
  <nav className={s.toolbar} aria-label="Operations controls"><div className={s.identity}><span>NETWORK CONTROL</span><strong>Operations</strong></div><button aria-expanded={events} onClick={()=>{setEvents(!events);if(innerWidth<1100){setSelected(null);setSelectedLiveFlight(null)}}}>Events <b>{activeEvents.length}</b></button><button aria-expanded={compare} onClick={()=>{setCompare(!compare);setTimeline(false);setSelected(null);setSelectedLiveFlight(null)}}>Recovery <b>{recoveryPlans.length}</b></button><button onClick={reset} disabled={busy} aria-busy={busy}>{runId?"Back to demo":"Reset simulation"}</button><div className={s.search}><FlightSearch selectedFlight={selected} onSelect={select}/></div></nav>
  ...
  {events&&<aside className={s.events} aria-label="Disruption controls"><header><div><span>SIMULATION</span><h2>Disruptions</h2></div><button aria-label="Close events" onClick={()=>setEvents(false)}>Close</button></header>{runId?<><p>These disruptions belong to your saved run. Edit the scenario and solve again to change them.</p><Link href={`/app/scenarios/${scenarioId}/setup` as Route}>Edit disruptions</Link><ul>{activeEvents.map((event,i)=><li key={i}>{String(event.kind)}</li>)}</ul></>:<EventPanel/>}</aside>}

...
import { Search, ArrowUpRight, LayoutDashboard, SlidersHorizontal, X } from "lucide-react"
...
function Shell({ children }: { children: React.ReactNode }) {
  ...
  return <div className={styles.workspace} data-theme={theme==="system"?(systemDark?"dark":"light"):theme} data-density={density}>
    ...
    <header className={styles.header}>
      ...
      <nav aria-label="Workspace" className={styles.nav}>{routes.map(({ href, label, icon: Icon }) => <Link key={href} href={href} aria-current={pathname === href ? "page" : undefined}><Icon size={17}/>{label}</Link>)}<Link href={"/app/scenarios" as Route} aria-current={pathname.startsWith("/app/scenarios") ? "page" : undefined}>Scenarios</Link><Link href={"/app/account" as Route} aria-current={pathname === "/app/account" ? "page" : undefined}>Account &amp; API keys</Link></nav>
      <div className={styles.headerActions}><button onClick={() => command.current?.showModal()} aria-label="Search workspace"><Search size={19}/><kbd>Ctrl K</kbd></button><button onClick={() => preferences.current?.showModal()} aria-label="Workspace preferences"><SlidersHorizontal size={19}/></button><details ref={toolsMenu} className={styles.tools}><summary>Tools</summary><nav aria-label="Analysis tools">{!runId&&<AgentBubble/>}<Link href={"/app/account" as Route}>Account & API keys</Link><Link href={"/app/benchmarks" as Route}>Run history & benchmarks</Link><button onClick={()=>openAnalysis("plans")}>Plan timeline & explanation</button><button onClick={()=>openAnalysis("crew")}>Crew coverage & duty margins</button><button disabled={!!runId} onClick={()=>setBrief(true)}>Daily operations brief</button>{!runId&&analysisRoutes.map(([href,label])=><Link key={href} href={href as Route}>{label}</Link>)}</nav></details></div>
    </header>
